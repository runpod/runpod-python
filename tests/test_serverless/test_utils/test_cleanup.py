...
def test_clean_no_folders():
    ...
    '''
    with patch('shutil.rmtree') as mock_rmtree, patch('os.remove') as mock_remove, patch('os.path.exists', return_value=False):
        rp_cleanup.clean()
...
def test_clean_with_output_zip():
    ...
    '''
    with patch('shutil.rmtree') as mock_rmtree, patch('os.remove') as mock_remove, patch('os.path.exists', return_value=True):
        rp_cleanup.clean()
        assert mock_rmtree.call_count == 3
