...
def get_ssh_ip_port(pod):
    ...
    Tries up to 5 times with an incremental backoff if necessary.
    '''
    pod_ip = None
    pod_port = None
    ...
    for attempt in range(5):
        ...
        if attempt < 5:  # We don't want to sleep after the third attempt
            time.sleep(2 * (attempt + 1))  # Sleeps for 2, 4 seconds

    if pod_ip is None or pod_port is None:
        raise Exception("Failed to retrieve SSH IP and port after 5 attempts.")
