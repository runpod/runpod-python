...
def _patch_template_files(project_folder, model_name, model_type):
    """Patch placeholder values in template files after copying."""
    ...
    if model_name:
        handler_path = os.path.join(project_folder, "src/handler.py")
...
def _write_project_toml(
    project_folder, project_name, cuda_version, python_version, model_type, model_name,
    storage_id="",
):
    """Build and write the runpod.toml configuration file."""
    ...
    project_table.add("storage_id", storage_id)
...
def create_new_project(
    ...
    if not init_current_dir:
        ...
        _patch_template_files(project_folder, model_name, model_type)
    else:
        project_folder = os.getcwd()

    _write_project_toml(
        project_folder, project_name, cuda_version, python_version,
        model_type, model_name, storage_id=runpod_volume_id,
    )


def _build_api_server_script(project_path_uuid_dev, project_name, remote_project_path,
                             handler_path, pip_req_path):
    """Build the shell script that runs the API dev server with hot-reload."""
    return f"""
            pkill inotify
...
def _setup_remote_environment(ssh_conn, config, project_pod_id):
    """Create remote project directories and install the virtual environment."""
    project_path_uuid = (
    ...
    return project_path_uuid_dev, remote_project_path
...
def start_project():  # pylint: disable=too-many-locals, too-many-branches
    ...
    with SSHConnection(project_pod_id) as ssh_conn:
        project_path_uuid_dev, remote_project_path = _setup_remote_environment(
            ssh_conn, config, project_pod_id
        )
        ...
        launch_api_server = [_build_api_server_script(
            project_path_uuid_dev, project_name, remote_project_path,
            handler_path, pip_req_path,
        )]
