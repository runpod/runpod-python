...
def configure() -> None:
    """Read current fitness settings; setup errors are handled by the runner."""
    global \
        MIN_MEMORY_GB, \
        MIN_DISK_PERCENT, \
        MIN_CUDA_VERSION, \
        NETWORK_CHECK_TIMEOUT, \
        GPU_BENCHMARK_TIMEOUT
    ...
    if NETWORK_CHECK_TIMEOUT <= 0 or GPU_BENCHMARK_TIMEOUT <= 0:
        raise ValueError(
            "RUNPOD_NETWORK_CHECK_TIMEOUT and RUNPOD_GPU_BENCHMARK_TIMEOUT must be positive"
        )
...
async def _check_network_connectivity() -> None:
    ...
    This is a worker-start readiness check, never an import-time hard failure.
    ...
    async def probe() -> None:
        ...
        try:
            ...
            # Bound connection teardown too; a stuck close must not hang startup.
            if writer.transport:
    ...
    for attempt in range(3):
        ...
        if remaining <= 0:
            break
        try:
            await asyncio.wait_for(probe(), timeout=remaining / (3 - attempt))
            log.info(f"Network connectivity passed: Connected to {host}:{port}")
            return
        ...
        if attempt < 2:
            await asyncio.sleep(
                min(0.1 * (attempt + 1), max(0, deadline - time.monotonic()))
            )
    raise RuntimeError(
        f"Network connectivity failed: {last_error} connecting to {host}:{port} "
        f"after bounded retries ({NETWORK_CHECK_TIMEOUT}s budget)"
    )
